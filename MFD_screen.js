class MFD_screen extends (typeof BaseInstrument !== "undefined" ? BaseInstrument : class { }) {
    constructor() {
        ...
        this.pages = ["primary", "map"]; // Pages supported by the MFD.
        this.pageDotsTimeout = null; // Timeout handler for hiding the page dots

        // RMI and knob-related setup
        this.RMI1Source = 2; // Default 2 = VOR1
        this.RMI2Source = 2; // Default 2 = VOR2
        this.bearing1 = NaN;
        this.bearing2 = NaN;
        ...
        // Navigation source initialization
        this.selectedNavSource = "VOR1"; // Default navigation source
        this.lastNavSource = "VOR1"; // Remember the last nav source
        this.gpsHasGP = false;
        this.nav1loc = false;
        this.nav2loc = false;

        // Flight and distance-related fields
        this.waypointIdent = ""; // Waypoint identifier
        this.identColor = "#e049b0"; // Ident color
        this.phaseText = "";
        this.distanceToNext = "";
        this.eteToNext = "";
        this.eteToNextStr = "";

        // HSI and navigation text
        this.navSourceText = "";
        this.vtgType = ""; // Vertical guidance type (e.g., "GS", "GP", "VNAV").
        this.vtgValid = false;
        this.vtgDeflection = 0;

        // Touch-sensitive inputs on the instrument
        this.touchBoxes = [
            { id: "trk", x: 90, y: 220, w: 40, h: 40 }, // HSI boxes
            { id: "crs", x: 190, y: 220, w: 40, h: 40 },
            { id: "cdi", x: 140, y: 230, w: 40, h: 30 },
            { id: "large_knob_cw", x: 0, y: 265, w: 20, h: 40 }, // Knob inputs
        ...
        // Image paths for bezel, compass, etc.
        ...
        // Image loading (including on-error fallback)
        this.images = {};
        ...
        for (const entry of imageMap) {
            ...
            this.images[entry.key].onload = () => {
                this.Update && this.Update();
            };
            this.images[entry.key].onerror = function () {
    ...
    getSimVars() {
        if (typeof SimVar !== "undefined" && typeof SimVar.GetSimVarValue === "function") {
            ...
            // Waypoint data
            this.previousWaypointLat = SimVar.GetSimVarValue("GPS WP PREV LAT", "degrees");
            this.previousWaypointLon = SimVar.GetSimVarValue("GPS WP PREV LON", "degrees");
            this.nextWaypointLat = SimVar.GetSimVarValue("GPS WP NEXT LAT", "degrees");
            this.nextWaypointLon = SimVar.GetSimVarValue("GPS WP NEXT LON", "degrees");
            this.nextNextWaypointLat = SimVar.GetSimVarValue("GPS WP AFTER NEXT LAT", "degrees");
            this.nextNextWaypointLon = SimVar.GetSimVarValue("GPS WP AFTER NEXT LON", "degrees");

            // Debug for light adjustments
            window.testHeading = this.heading; // Useful for debugging
        }
        // --- TEST MODE (sliders/controls from browser window) ---
        else {
            // Mocked values for testing in HTML
            this.heading = window.testHeading || 0;
            this.airplanePositionX = window.testAirplanePositionX || 0;
            this.airplanePositionY = window.testAirplanePositionY || 0;
            this.waypoints = [
                { x: window.testWaypoint1X || -50, y: window.testWaypoint1Y || -50 },
                { x: window.testWaypoint2X || 50, y: window.testWaypoint2Y || 50 },
            ];
    ...
    connectedCallback() {
        ...
        // Attach the canvas for rendering if not already done
        ...
        // Refresh the MFD screen periodically
    ...
    pollMFDKnobDeltas() {
        ...
        if (smallDelta !== 0) {
            ...
            // Determine step size (faster spin = larger step)
            let step = Math.abs(smallDelta) > 1 ? Math.abs(smallDelta)
                : (timeSince < 70 ? 10 : timeSince < 250 ? 3 : 1);

            this.handleKnobDelta(step * Math.sign(smallDelta), "small");
            SimVar.SetSimVarValue("L:MFD_KnobSmallDelta", "number", 0); // RESET SMALL DELTA
        }

        // LARGE KNOB DELTA HANDLING
        ...
        if (largeDelta !== 0) {
            ...
            // Determine step size (faster spin = larger step)
            let step = Math.abs(largeDelta) > 1 ? Math.abs(largeDelta)
                : (timeSince < 70 ? 10 : timeSince < 250 ? 3 : 1);

            // Navigate pages based on the large knob's delta
            if (largeDelta > 0) this.nextPage(); // Clockwise increases page
            else if (largeDelta < 0) this.previousPage(); // Counterclockwise decreases page

            SimVar.SetSimVarValue("L:MFD_KnobLargeDelta", "number", 0); // RESET LARGE DELTA
        }
    }
    ...
    showPageDots() {
        const pageDotsElement = document.getElementById('pageDots');
        if (pageDotsElement) {
            pageDotsElement.style.display = 'block'; // Show the dots
            clearTimeout(this.pageDotsTimeout); // Clear existing timeout if any

            // Hide the dots after 5 seconds
            this.pageDotsTimeout = setTimeout(() => {
                pageDotsElement.style.display = 'none'; // Hide the dots
            }, 5000);
        }
    }
    ...
    Update() {
        this.getSimVars(); // Retrieve simulation values.
        this.followTRK(); // Update heading tracking/TRK logic.

        // Validate and prepare the canvas.
        const canvas = this.canvas || (this.canvas = document.getElementById("mfdCanvas"));
        if (!canvas) return;

        const ctx = this.canvasCacheContext || (this.canvasCacheContext = canvas.getContext("2d"));
        const { width: w, height: h } = canvas;
        const cx = w / 2, cy = h / 2;

        ctx.clearRect(0, 0, w, h); // Clear canvas for fresh drawing.
        ...
        if (this.currentPage === "primary") {
            this.renderPrimaryPage(ctx, cx, cy, w, h); // Delegate rendering for primary page.
        } else if (this.currentPage === "map") {
            this.renderMapPage(ctx);
        }

        // Render universal/common elements.
        this.renderCommonElements(ctx, w, h);
    }
    ...
    renderCommonElements(ctx, w, h) {
        this.drawBezel(ctx, w, h);
        this.drawPagingDots(ctx);
        this.drawTouchBoxes(ctx, true); // Optional: Highlight interactive boxes.
        this.drawTASBox(ctx); // True Airspeed overlay.
    }
}
